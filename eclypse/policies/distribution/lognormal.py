"""Lognormal-distribution resource policy."""
...
def lognormal(
    *,
    ...
    node_distribution: tuple[float, float] = (0.0, 0.05),
    ...
    Args:
        ...
        node_distribution (tuple[float, float]): Default ``(mu, sigma)`` pair
            used for node multipliers.
        edge_distribution (tuple[float, float] | None): Default ``(mu, sigma)``
    ...
    Returns:
        UpdatePolicy: A graph update policy applying lognormal multiplicative
            noise.
    """
