"""Exponential multiplier distribution policy."""
...
def exponential(
    *,
    ...
    Args:
        ...
        node_distribution (float): Default rate for selected node assets.
        edge_distribution (float | None): Default rate for selected edge assets. When omitted,
            ``node_distribution`` is reused.
        node_asset_distributions (dict[str, float] | None): Optional per-node-asset rates.
        edge_asset_distributions (dict[str, float] | None): Optional per-edge-asset rates.
    ...
    validate_distributions(
        ...
        checks=_BUILTIN_DISTRIBUTION_CHECKS["exponential"],
    )
