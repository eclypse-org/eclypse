...
def reduce(
    *,
    ...
    Args:
        factor (float | None):
            Relative multiplicative factor applied to each selected asset.
            Provide either ``factor`` or ``target``.
        target (float | None):
            Absolute value reached by each selected asset at the end of the
            adjustment horizon. Provide either ``factor`` or ``target``.
        epochs (int | None):
            Number of evolution steps over which the reduction is applied.
        node_assets (str | list[str] | None):
            Node asset names using the default adjustment
            configuration.
        edge_assets (str | list[str] | None):
            Edge asset names using the default adjustment
            configuration.
        ...
        node_filter (NodeFilter | None):
            Optional predicate used to select nodes dynamically.
