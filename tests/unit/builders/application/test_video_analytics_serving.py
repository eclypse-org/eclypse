...
from eclypse.builders.application import get_video_analytics_serving
from eclypse.remote.service.service import Service


def test_video_analytics_serving_builder():
    plain_app = get_video_analytics_serving(include_default_assets=True)
    mpi_app = get_video_analytics_serving(
        include_default_assets=True,
        communication_interface="mpi",
    )
    rest_app = get_video_analytics_serving(
        include_default_assets=True,
    ...
    assert plain_app.has_edge("CameraGatewayService", "DetectionService")
    assert len(plain_app.flows) == 2


def test_video_analytics_serving_rejects_unknown_interfaces():
    with pytest.raises(ValueError, match="Unknown communication interface"):
        get_video_analytics_serving(communication_interface="grpc")  # type: ignore[arg-type]
