...
from eclypse.builders.application import get_media_service
from eclypse.remote.service.service import Service


def test_media_service_builder():
    plain_app = get_media_service(include_default_assets=True)
    mpi_app = get_media_service(
        include_default_assets=True,
        communication_interface="mpi",
    )
    rest_app = get_media_service(
        include_default_assets=True,
    ...
    assert plain_app.has_edge("ComposeReviewService", "UniqueIdService")
    assert len(plain_app.flows) == 4


def test_media_service_rejects_unknown_interfaces():
    with pytest.raises(ValueError, match="Unknown communication interface"):
        get_media_service(communication_interface="grpc")  # type: ignore[arg-type]
